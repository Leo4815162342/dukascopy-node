...
describe('Buffer fetcher with retry logic', () => {
  ...
  beforeEach(() => {
    mockFetch
      ...
      .mockReturnValueOnce(
        //@ts-ignore
        Promise.resolve({ buffer: async () => Buffer.from('try 1 - fail', 'utf-8'), status: 503 })
      )
      // second call
      .mockReturnValueOnce(
        //@ts-ignore
        Promise.resolve({ buffer: async () => Buffer.from('try 2 - fail', 'utf-8'), status: 503 })
      )
      // third call
      .mockReturnValueOnce(
        //@ts-ignore
        Promise.resolve({ buffer: async () => Buffer.from('try 3 - fail', 'utf-8'), status: 503 })
      )
      .mockReturnValue(
        //@ts-ignore
        Promise.resolve({
